<template>
  <div class="mx-auto max-w-6xl space-y-8">
    ...
    <div>
      ...
      <p class="mt-1 text-sm text-slate-500">Lanjutkan belajar dan capai targetmu hari ini.</p>
    </div>

    <!-- Stats -->
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <StatCard label="Kursus Aktif" :value="3" subtitle="2 sedang berjalan" :icon="iconBook" />
      <StatCard label="Progress Belajar" :value="'68%'" subtitle="Kerja bagus!" :icon="iconChart" iconBg="bg-emerald-100" iconColor="text-emerald-600" />
      <StatCard label="Pelajaran Selesai" :value="24" subtitle="dari 36 pelajaran" :icon="iconCheck" iconBg="bg-blue-100" iconColor="text-blue-600" />
      <StatCard label="Hari Aktif" :value="12" subtitle="bulan ini" :icon="iconCalendar" iconBg="bg-amber-100" iconColor="text-amber-600" />
    </div>
    ...
    <div>
      <div class="mb-4 flex items-center justify-between">
        ...
        <RouterLink to="/client/my-courses" class="text-xs font-bold text-[#cf3d3d] hover:underline">Lihat semua →</RouterLink>
      </div>
        ...
        <Card v-for="course in courses" :key="course.id" class="overflow-hidden">
          <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
            ...
            <div class="flex-1">
              ...
              <p class="mt-0.5 text-xs text-slate-400">Lesson {{ course.lesson }} dari {{ course.totalLessons }}</p>
              <div class="mt-3">
          ...
          <div class="border-t border-slate-100 px-5 py-3">
            <RouterLink :to="`/client/my-courses/${course.id}/learn`" class="text-xs font-bold text-[#cf3d3d] hover:underline">
              Lanjutkan →
            </RouterLink>
          </div>
        </Card>
    ...
    <div>
      <div class="mb-4 flex items-center justify-between">
        ...
        <RouterLink to="/client/orders" class="text-xs font-bold text-[#cf3d3d] hover:underline">Riwayat →</RouterLink>
      </div>
      <Card class="divide-y divide-slate-50">
        <div v-for="order in orders" :key="order.id" class="flex items-center justify-between p-5">
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-sm font-bold text-slate-800">{{ order.package }}</p>
              <p class="text-xs text-slate-400">{{ order.orderNumber }}</p>
            </div>
          </div>
          <div class="text-right">
            <p class="text-sm font-extrabold text-slate-800">{{ order.amount }}</p>
...
const orders = [
  { id: 1, package: 'Paket Premium Online', orderNumber: 'NA-001-abcd1234', amount: 'Rp199.000', status: 'paid' },
  { id: 2, package: 'Paket Basic Online', orderNumber: 'NA-002-efgh5678', amount: 'Rp99.000', status: 'pending' },
]
</script>
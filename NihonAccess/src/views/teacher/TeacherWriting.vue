            ...
            <div v-for="row in items" :key="row.id" class="p-4">
              <div class="flex items-start justify-between gap-3">
                <div class="flex min-w-0 flex-1 items-center gap-3">
                  ...
                  <div class="min-w-0">
                    <p class="text-xs font-medium text-slate-500">{{ row.course?.title || '-' }}</p>
                ...
                <div class="flex shrink-0 gap-1.5">
                  <button @click="$router.push({ name: 'TeacherWritingEdit', params: { id: row.id } })" class="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-slate-500 transition-colors hover:border-[#cf3d3d]/40 hover:text-[#cf3d3d]" title="Edit">
                    <span v-html="iconEdit" />
                  </button>

    ...
    <div class="rounded-3xl border border-slate-100 bg-white shadow-sm">
      ...
      <template v-else>
        ...
        <div v-if="confirmId" class="flex items-center justify-between gap-4 border-t border-slate-100 bg-[#cf3d3d]/5 px-6 py-4">
          <p class="text-sm font-semibold text-slate-700">Hapus "<span class="font-jp text-[#cf3d3d]">{{ confirmChar }}</span>"?</p>
...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await teacherApi.writing.list({ page: page.value, per_page: 10, ...(filterType.value ? { character_type: filterType.value } : {}) })
    items.value = res.data
    meta.value = res.meta
  } catch (e) {
    error(e.message || 'Gagal memuat writing.')
  } finally {
    loading.value = false
  }
}
...
const confirmDelete = async () => {
  deleting.value = true
  try {
    await teacherApi.writing.remove(confirmId.value)
    success('Writing berhasil dihapus.')
    confirmId.value = null
    fetchItems()
  } catch (e) {
    error(e.message || 'Gagal menghapus writing.')
  } finally {

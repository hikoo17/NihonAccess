<template>
  <div class="animate-fadeIn">
    <TeacherPageHeader
      eyebrow="Writing (Tebak Huruf)"
      :title="isEdit ? 'Edit Writing' : 'Tambah Writing'"
      subtitle="Latihan tebak huruf dengan urutan goresan."
      :back-to="{ name: 'TeacherWriting' }"
    >
      <template #actions>
        <Button variant="secondary" size="sm" @click="$router.push({ name: 'TeacherWriting' })">Batal</Button>
        ...
        <form v-else class="space-y-5" @submit.prevent="submit">
          ...
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input v-model="form.character" type="text" placeholder="cth. ã‚" :class="formInputClass(errors.character)" />
              <p v-if="errors.character" class="mt-1 text-xs font-semibold text-[#cf3d3d]">{{ errors.character[0] }}</p>
            </div>
          </div>

          <div>
            <label class="mb-1.5 block text-sm font-bold text-slate-700">URL Gambar Urutan Goresan</label>
            <input v-model="form.stroke_order_image" type="url" placeholder="https://..." :class="formInputClass(errors.stroke_order_image)" />
            <p class="mt-1 text-xs font-medium text-slate-400">Gambar panduan urutan goresan menulis karakter.</p>
            <p v-if="errors.stroke_order_image" class="mt-1 text-xs font-semibold text-[#cf3d3d]">{{ errors.stroke_order_image[0] }}</p>
          </div>
...
const buildPayload = () => ({
  ...
  stroke_order_image: form.stroke_order_image || null,
...
const submit = async () => {
  ...
  try {
    if (isEdit.value) {
      await teacherApi.writing.update(editId, buildPayload())
      success('Writing berhasil diperbarui.')
    } else {
      await teacherApi.writing.create(buildPayload())
      success('Writing berhasil dibuat.')
    }
    router.push({ name: 'TeacherWriting' })
  } catch (e) {
    if (e.errors) errors.value = e.errors
    error(e.message || 'Gagal menyimpan writing.')
  } finally {

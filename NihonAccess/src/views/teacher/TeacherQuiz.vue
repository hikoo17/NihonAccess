<template>
  <div class="animate-fadeIn">
    <TeacherPageHeader
      eyebrow="Quiz & Soal"
      title="Quiz & Soal"
      subtitle="Kelola kuis beserta daftar soal pilihan ganda."
    >
      <template #actions>
        <div class="relative">
          <input v-model="search" type="text" placeholder="Cari quiz..." class="h-10 w-48 rounded-2xl border border-slate-200 bg-white pl-9 pr-3 text-sm font-medium text-slate-700 placeholder:text-slate-400 transition focus:border-[#cf3d3d] focus:outline-none focus:ring-2 focus:ring-[#cf3d3d]/20" />
    ...
    <div class="rounded-3xl border border-slate-100 bg-white shadow-sm">
      ...
      <template v-else>
        ...
        <div v-if="confirmId" class="flex items-center justify-between gap-4 border-t border-slate-100 bg-[#cf3d3d]/5 px-6 py-4">
          <p class="text-sm font-semibold text-slate-700">Hapus quiz "<span class="text-[#cf3d3d]">{{ confirmTitle }}</span>"?</p>
...
watch(search, () => {
  ...
  searchTimer = setTimeout(() => { page.value = 1; fetchQuizzes() }, 350)
})

const fetchQuizzes = async () => {
  loading.value = true
  try {
    const res = await teacherApi.quizzes.list({ page: page.value, per_page: 10, ...(search.value ? { search: search.value } : {}) })
    items.value = res.data
    meta.value = res.meta
  } catch (e) {
    error(e.message || 'Gagal memuat quiz.')
  } finally {
    loading.value = false
  }
}

const onPage = (p) => { page.value = p; fetchQuizzes() }
...
const confirmDelete = async () => {
  deleting.value = true
  try {
    await teacherApi.quizzes.remove(confirmId.value)
    success('Quiz berhasil dihapus.')
    confirmId.value = null
    fetchQuizzes()
  } catch (e) {
    error(e.message || 'Gagal menghapus quiz.')
  } finally {
